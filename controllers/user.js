...
exports.resetPasswordSend = function(req, res) {
  User.findOne({
    ...
    if (user) {
      ...
      if (!user.verified) {
        res.status(500).json({
          message: 'Account not verified yet. Please verify your account first by opening the link in your verification email.',
          verify: true
        });
      } else {
        // Update existing user with a new token
...
function sendVerificationEmail(options, done) {
  ...
  if (process.env.POSTMARK_API_KEY) {
    let emailClient = new postmark.ServerClient(process.env.POSTMARK_API_KEY);
